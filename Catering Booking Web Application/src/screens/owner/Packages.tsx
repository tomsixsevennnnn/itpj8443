...
export default function Packages({
  ...
  /* --- ลากจัดเรียงแพ็กเกจในตาราง --- */

  const handlePkgDragStart = (id: string) => setDragPkgId(id)

  const handlePkgDragOver = (e: React.DragEvent) => e.preventDefault()

  const handlePkgDrop = (targetId: string) => {
    const dragId = dragPkgId
    setDragPkgId(null)
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await onUpdatePackage(editing.id, base)
      } else {
  ...
  return (
    <div>
      ...
      {packages.length > 1 && (
        <p className="text-xs text-gray-400 mb-2">ลากแถวเพื่อจัดลำดับแพ็กเกจที่ลูกค้าจะเห็น</p>
      )}
          ...
          <tbody className="divide-y divide-gray-50">
            {packages.map((pkg) => (
              <tr
                key={pkg.id}
                draggable
                onDragStart={() => handlePkgDragStart(pkg.id)}
                onDragOver={handlePkgDragOver}
                onDrop={() => handlePkgDrop(pkg.id)}
                onDragEnd={() => setDragPkgId(null)}
                className={`hover:bg-gray-50/50 transition-colors cursor-grab active:cursor-grabbing ${
                  dragPkgId === pkg.id ? 'opacity-40' : ''
                }`}
              >
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[92vh] flex flex-col overflow-hidden">
            ...
            <div className="border-t border-gray-100 px-6 py-4 flex items-center gap-3 flex-shrink-0">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 disabled:bg-gray-200 disabled:text-gray-400 text-white rounded-xl px-5 py-2.5 text-sm font-semibold transition-colors shadow-lg shadow-orange-200 disabled:shadow-none"
              >
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-6">
            ...
            <p className="text-sm text-gray-500">
              "{confirmDelete.name}" — ลบแล้วจะไม่สามารถกู้คืนได้
            </p>
